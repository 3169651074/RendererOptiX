...
namespace project {
    ...
    std::vector<GAS> buildAddDataGAS(RendererData & data) {
    ...
    void readVTKFileCache(
            ...
            const std::vector<GAS> & addGAS, RendererData & data)
    {
        ...
        //读取缓存文件并直接移动到全局数组
        data.vtkParticleAllFiles[currentFileIndex] = VTKReader::readVTKDataCache(currentCacheFilePathName);
            ...
            const std::string & cacheFilePath, bool isDebugMode)
    {
        ...
        auto [vtkFiles, durations, fileCount] = VTKReader::readSeriesFile(seriesFilePath, seriesFileName);
        ...
        const auto maxCellCountSingleFile = VTKReader::readMaxCellCountAllVTKFile(cacheFilePath);
    ...
    void Renderer::freeRendererData(RendererData & data) {
        ...
        for (auto & vtkParticleThisFile: data.vtkParticleAllFiles) {
            VTKReader::freeVTKData(vtkParticleThisFile);
        }
