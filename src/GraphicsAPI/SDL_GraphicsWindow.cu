...
namespace project {
            ...
            float cameraMoveSpeedStride, size_t initialSpeedNTimesStride, bool useDebugMode)
    {
        ...
        args.type = type;
        args.cameraMoveSpeed = cameraMoveSpeedStride * static_cast<float>(initialSpeedNTimesStride);
    ...
    void SDL_GraphicsWindowUpdateCamera(
        ...
        if (dx != 0 || dy != 0) {
            ...
            const float yawAngle = -static_cast<float>(dx) * args.mouseSensitivity;
            W = rotate(W, V, yawAngle);
            float pitchAngle = -static_cast<float>(dy) * args.mouseSensitivity;
            W = rotate(W, U, pitchAngle);

            const float pitchLimitRadian = MathHelper::degreeToRadian(args.pitchLimitDegree);
            //使用cameraV（up方向）来计算pitch，而不是硬编码y轴
            float newPitch = std::asin(dot(W, V));
            ...
            if (needsCorrection) {
                //使用cameraU和cameraV来定义水平和垂直方向
                //水平方向：垂直于V（up方向）的方向
                const float3 horizontalDir = normalize(W - V * dot(W, V));
                const float horizontalMagnitude = std::cos(newPitch);
                W = horizontalDir * horizontalMagnitude + V * std::sin(newPitch);
            }
        ...
        //移除W在worldUp（世界的up方向）上的分量，得到水平方向
        const float3 W_for_movement = normalize(camera.cameraW);
        const float3 worldUp = normalize(camera.upDirection);
        const float3 projected = W_for_movement - worldUp * dot(W_for_movement, worldUp);
        const float projectedLength = length(projected);
        //如果投影后向量长度太小（相机几乎垂直向上或向下看），使用cameraU作为前方向
        const float3 forwardHorizontal = projectedLength > FLOAT_ZERO_VALUE ? normalize(projected) : normalize(camera.cameraU);
