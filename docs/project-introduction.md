...
RendererOptiX 是一个基于 NVIDIA OptiX 9.0 实现的高性能实时渲染器，专门用于渲染 VTK（Visualization Toolkit）格式的粒子数据。该项目结合了现代光线追踪技术和多种图形 API，提供了一个灵活且高效的渲染解决方案。
